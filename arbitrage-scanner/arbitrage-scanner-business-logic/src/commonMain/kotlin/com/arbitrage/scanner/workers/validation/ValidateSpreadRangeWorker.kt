 ...
 *
 * Правила валидации:
 * - minSpread не может быть отрицательным (если указан)
 * - maxSpread не может быть отрицательным (если указан)
 * - maxSpread не может быть больше 100 (если указан)
 * - Если указаны оба значения, то minSpread должен быть <= maxSpread
 */
fun ICorAddExecDsl<Context, BusinessLogicProcessorImplDeps>.validateSpreadRangeWorker(
    ...
    on {
        ...
        val hasNegativeMin = minSpread != null && minSpread < 0
        val hasNegativeMax = maxSpread != null && maxSpread < 0
        val hasMaxOver100 = maxSpread != null && maxSpread > 100

        // Проверяем корректность диапазона
        val hasInvalidRange = minSpread != null && maxSpread != null && minSpread > maxSpread
    ...
    handle {
        ...
        when {
            ...
            minSpread != null && maxSpread != null && minSpread > maxSpread -> {
                fail(
                    InternalError(
                        code = "validation-range",
                        group = "validation",
